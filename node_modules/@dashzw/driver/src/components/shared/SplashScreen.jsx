...
/**
 * SplashScreen — shown briefly after login.
 * Uses a ref for onDone so parent re-renders do not reset the 3s timer.
 */
export default function SplashScreen({ onDone }) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4">
        ...
        <div
          className={`overflow-hidden transition-all duration-500 ${
            phase >= 1 ? 'max-w-[200px] opacity-100' : 'max-w-0 opacity-0'
          }`}
        >
          <div className={`transition-transform duration-500 ${phase >= 1 ? 'translate-x-0' : 'translate-x-8'}`}>
            ...
            <p className="text-white/70 text-sm font-medium tracking-wider mt-1 whitespace-nowrap">
              Food delivery, made local
            </p>
          </div>
        </div>
      </div>
      ...
      >
        <p className="text-white/50 text-xs tracking-widest uppercase">Delivering happiness</p>
      </div>
    </div>
  );
}
