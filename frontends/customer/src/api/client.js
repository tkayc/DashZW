...
export async function apiFetch(path, options = {}) {
  ...
  if (!res.ok) {
    ...
    err.existingEmail = data.existingEmail;
    throw err;
  }
  return data;
}
...
export async function getCollection(name, limit = 100) {
  ...
  if (!collectionLoading[cacheKey]) {
    collectionLoading[cacheKey] = apiFetch(
      ...
      // Keep legacy cache key for sync reads expecting default preload
...
export function createApiClient() {
  const auth = {
    ...
    checkAccount: async (payload) =>
      apiFetch('/api/auth/check-account', { method: 'POST', body: JSON.stringify(payload) }),
